...
gesnipet werden (dieselben Kanäle wie bei !add/!list), und gleicht das mit
den ECHTEN eigenen Verkäufen aus der Buchhaltung ab – KEINE manuelle Pflege
nötig, läuft von selbst sobald der Snipe-Bot Kanäle mit Suchen hat.

Zwei Datenquellen:
  1. Angebots-Trend (Vinted): wie viele aktive Inserate es pro Marke/Kategorie
     gerade gibt, und ob das steigt oder fällt. Proxy für Konkurrenz/Interesse,
     KEINE echte Verkaufszahl – die gibt Vinted öffentlich nicht her.
...
def _effective_terms() -> list[str]:
    """Automatische Snipe-Bot-Begriffe + manuell mit !trends-add hinzugefügte
    Extras, dedupliziert (Groß-/Kleinschreibung egal)."""
    seen: set[str] = set()
    merged: list[str] = []
    for term in _auto_terms() + [r["term"] for r in _fetch_watchlist()]:
        key = term.lower()
...
class Trends(commands.Cog):
    ...
    async def trends_liste(self, ctx: commands.Context):
        ...
        if not auto and not manuell:
            await ctx.send(
                "📭 Aktuell wird nichts getrackt — leg mit `!add <kanal> <vinted-url>` einen "
                "Snipe-Bot-Kanal an (der wird automatisch mitgetrackt) oder füge manuell einen "
                "Begriff mit `!trends-add <Begriff>` hinzu."
            )
            return
        ...
        embed.set_footer(text=f"Tägliches Update + Sonntags-Recap in #{TRENDS_CHANNEL_NAME}")
        await ctx.send(embed=embed)
    ...
    async def trends_hilfe(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="Läuft automatisch über deine Snipe-Bot-Kanäle — kein Setup nötig. "
                        "Die folgenden Befehle sind nur für Ad-hoc-Checks oder Extras.",
            color=COLOR,
        )
